...
- OPENROUTER_API_KEY is set. Check with echo $OPENROUTER_API_KEY. If empty, the user must get a free key at [https://openrouter.ai/keys](https://openrouter.ai/keys) and set it:
```
...
2. Restart gateway so OpenClaw picks up the changes
   ```
...
| OPENROUTER_API_KEY not set | User needs a key from [https://openrouter.ai/keys](https://openrouter.ai/keys) |
